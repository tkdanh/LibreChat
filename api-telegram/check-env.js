...
if (fs.existsSync(rootEnvPath)) {
  ...
  const hasTelegramToken = content.includes("TELEGRAM_BOT_TOKEN");
  const hasApiToken = content.includes("API_TOKEN");
  console.log(`   ✓ Exists`);
  console.log(`   - Has TELEGRAM_BOT_TOKEN: ${hasTelegramToken ? "YES" : "NO"}`);
  console.log(`   - Has API_TOKEN: ${hasApiToken ? "YES" : "NO"}`);
  if (!hasTelegramToken || !hasApiToken) {
...
console.log(`\n2. Local api-telegram/.env: ${localEnvPath}`);
...
if (!fs.existsSync(rootEnvPath)) {
  ...
  if (!rootContent.includes("TELEGRAM_BOT_TOKEN") || !rootContent.includes("API_TOKEN")) {
    ...
    if (fs.existsSync(localEnvPath)) {
      ...
      if (localContent.trim().length === 0) {
        console.log("⚠ api-telegram/.env exists but is empty. You can delete it or add variables there.");
      }
    }
  }
}
